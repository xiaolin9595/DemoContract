...
contract StructSet {
    ...
    function _get(bytes32 _digest) internal view returns(bool exists, address account) {
        account = _accountList[_digest]._account;
        exists = (account != address(0));
    }

    function _genDigest(Account memory accList) internal pure returns (bytes32 digest) {
        return keccak256(accList._pubkey);
    }

    function _str2bytes(string memory str) internal pure returns (bytes32 result) {
        assembly {
            result := mload(add(str, 32))   
        }
    }

    function _bytes2str(bytes memory dat) internal pure returns (string memory) {
...
contract AccountList is IAccountList, StructSet, Ownable {
    ...
    function Add(bytes memory FIDOPubkey, address accountAddress) external returns(bool success) {
        require(msg.sender==_caller,"Only the specified entry point can  call Add()");
